<template>
  ...
  <ContentWrap>
    ...
    >
      ...
      <el-table-column align="center" label="是否是主播" prop="anchorStatus">
          <template #default="scope">
            <el-switch
              v-model="scope.row.anchorStatus"
              :active-value="true"
      ...
      <el-table-column align="center" label="活力值" prop="vitality" />
      <el-table-column align="center" label="余额" prop="walletNum" />
      ...
      >
        <template #default="scope">
          <div class="flex items-center justify-center">
            ...
            <el-button link type="primary" v-if="scope.row.anchorStatus" @click="updateDrawRate(scope.row)">更新费率</el-button>
            <!-- <el-dropdown
              v-hasPermi="[
                'member:user:update',
  ...
  <Dialog title="更新费率" v-model="dialogVisible">
    <el-form>
      <el-form-item label="费率" prop="drawRate">
        <el-input-number
          v-model="drawRate"
          class="!w-240px"
          clearable
...
const drawRate = ref(0)
const currentId = ref('')
const updateDrawRate = (row)=>{
  dialogVisible.value = true
  currentId.value = row.id
  drawRate.value = row.drawRate
}
const submitForm = async () => {
    await UserApi.updateUserDrawRate({id:currentId.value,drawRate:drawRate.value})
...
const handleStatusChange = async (row) => {
  try {
    // 修改状态的二次确认
    const text = row.anchorStatus ? '启用' : '关闭'
    await message.confirm('确认要"' + text + '""' + row.name + '"为主播吗?')
    // 发起修改状态
    await UserApi.updateUserStatus(row.id, row.anchorStatus)
    // 刷新列表
    await getList()
  } catch {
    // 取消后，进行恢复按钮
